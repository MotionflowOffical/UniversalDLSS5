...
int main(){
 ...
 bool ok=true;
 ok&=need(cm,"project(UniversalDLSS5 VERSION 0.3.2","CMake version is not v0.3.2");
 ok&=need(bw,"UniversalDLSS5 v0.3.2","Windows build banner is stale");
 ok&=need(br,"UniversalDLSS5 v0.3.2","release build banner is stale");
 ok&=need(ui,"v0.3.2","controller UI version is stale");
 ok&=need(rc,"FILEVERSION 0,3,2,0","Windows file version is stale");
 ok&=need(rc,"\"0.3.2.0\\0\"","Windows string version is stale");
 ok&=need(host,"0.3.2","NRHost engine version is stale");
 ok&=need(ngx,"0.3.2","NGX engine version is stale");
 ok&=need(sl,"UniversalDLSS5-0.3.2","Streamline engine version is stale");

...
enum ButtonId : int {
    ...
    BTN_RETRY_NEURAL, BTN_COPY_DIAG, BTN_SAVE_DIAG, BTN_OPEN_LOGS, BTN_IMPORT_NVIDIA, BTN_OPEN_RUNTIME, BTN_NVIDIA_DOWNLOAD
};
enum ToggleId : int {
...
enum ChoiceId : int {
    ...
    CH_MOTION, CH_LATENCY, CH_DOWNSAMPLE, CH_DEPTH, CH_DEBUG, CH_THEME, CH_HDR
};
enum SliderId : int {
...
struct State {
    ...
    int selectedIndex{-1};
    int appListScroll{};
    D2D1_RECT_F appListViewport{};
    bool refreshingApps{};
    std::uint64_t refreshStartedMs{};
    ...
    std::atomic_bool autoDetachedPending{false};
...
std::wstring controllerPreferencesPath() {
    wchar_t base[32768]{};
    const DWORD n=GetEnvironmentVariableW(L"LOCALAPPDATA",base,_countof(base));
    return (fs::path(n?std::wstring(base,n):g.appDir)/L"UniversalDLSS5"/L"controller.ini").wstring();
}

void loadControllerPreferences() {
    std::ifstream in(fs::path(controllerPreferencesPath()),std::ios::binary);
    if(!in) return;
    std::string text((std::istreambuf_iterator<char>(in)),{});
    ControllerPreferences prefs{};
    if(decodeControllerPreferences(text,prefs)) g.settings.uiTheme=prefs.uiTheme;
}

void saveControllerPreferences() {
    const auto path=fs::path(controllerPreferencesPath());
    std::error_code ec;
    fs::create_directories(path.parent_path(),ec);
    std::ofstream out(path,std::ios::binary|std::ios::trunc);
    if(!out) return;
    ControllerPreferences prefs{};
    prefs.uiTheme=g.settings.uiTheme;
    const auto text=encodeControllerPreferences(prefs);
...
std::wstring rendererModuleSummary(std::uint32_t mask){
    std::wstring out;auto add=[&](const wchar_t* n){if(!out.empty())out+=L"/";out+=n;};
    if(mask&AppRendererD3D9)add(L"D3D9");if(mask&AppRendererD3D10)add(L"D3D10");if(mask&AppRendererD3D11)add(L"D3D11");if(mask&AppRendererD3D12)add(L"D3D12");if(mask&AppRendererOpenGL)add(L"OpenGL");if(mask&AppRendererVulkan)add(L"Vulkan");
    return out;
}

std::wstring processDetail(const PickerEntry*e){if(!e)return L"No application selected.";std::wstringstream out;if(e->grouped){out<<e->processCount<<L" process"<<(e->processCount==1?L"":L"es");if(e->visibleWindowCount>1)out<<L" \u00B7 "<<e->visibleWindowCount<<L" windows";}else out<<L"PID "<<e->root.pid;out<<L" \u00B7 "<<archName(e->root.arch);const auto renderers=rendererModuleSummary(e->rendererModules|e->root.rendererModules);if(!renderers.empty())out<<L" \u00B7 "<<renderers;else if(e->anyDxgi||e->root.hasDxgi)out<<L" \u00B7 DXGI detected";else out<<L" \u00B7 renderer not detected yet";if(e->root.blocksThirdPartyModules)out<<L" \u00B7 signed-module policy (CIG)";return out.str();}
...
const wchar_t* graphicsApiText(GraphicsApi a){switch(a){case GraphicsApi::D3D9:return L"D3D9";case GraphicsApi::D3D10:return L"D3D10";case GraphicsApi::D3D11:return L"D3D11";case GraphicsApi::D3D12:return L"D3D12";case GraphicsApi::OpenGL:return L"OpenGL";case GraphicsApi::Vulkan:return L"Vulkan";default:return L"Unknown";}}
...
const wchar_t* hdrEncodingText(std::uint32_t e){switch((ColorEncoding)e){case ColorEncoding::SdrLinear:return L"SDR linear/UNORM";case ColorEncoding::SdrSrgb:return L"SDR sRGB";case ColorEncoding::Hdr10Pq:return L"HDR10 PQ / BT.2020";case ColorEncoding::ScRgb:return L"scRGB linear";case ColorEncoding::UnsupportedHdr:return L"Unsupported HDR";default:return L"Unknown";}}
const wchar_t* swapFormatText(std::uint32_t f){switch((DXGI_FORMAT)f){case DXGI_FORMAT_R8G8B8A8_UNORM:return L"R8G8B8A8_UNORM";case DXGI_FORMAT_R8G8B8A8_UNORM_SRGB:return L"R8G8B8A8_UNORM_SRGB";case DXGI_FORMAT_B8G8R8A8_UNORM:return L"B8G8R8A8_UNORM";case DXGI_FORMAT_B8G8R8A8_UNORM_SRGB:return L"B8G8R8A8_UNORM_SRGB";case DXGI_FORMAT_R10G10B10A2_UNORM:return L"R10G10B10A2_UNORM";case DXGI_FORMAT_R16G16B16A16_FLOAT:return L"R16G16B16A16_FLOAT";default:return L"Other";}}
const wchar_t* colorSpaceText(std::uint32_t c){switch((DXGI_COLOR_SPACE_TYPE)c){case DXGI_COLOR_SPACE_RGB_FULL_G22_NONE_P709:return L"RGB_FULL_G22_NONE_P709";case DXGI_COLOR_SPACE_RGB_FULL_G10_NONE_P709:return L"RGB_FULL_G10_NONE_P709 (scRGB)";case DXGI_COLOR_SPACE_RGB_FULL_G2084_NONE_P2020:return L"RGB_FULL_G2084_NONE_P2020 (HDR10)";case DXGI_COLOR_SPACE_RGB_STUDIO_G2084_NONE_P2020:return L"RGB_STUDIO_G2084_NONE_P2020 (HDR10)";case DXGI_COLOR_SPACE_RGB_STUDIO_G22_NONE_P709:return L"RGB_STUDIO_G22_NONE_P709";case DXGI_COLOR_SPACE_RGB_STUDIO_G24_NONE_P709:return L"RGB_STUDIO_G24_NONE_P709";default:return L"Other/unknown";}}

std::wstring buildDiagnosticReport(const RuntimeStatus&s){std::wstringstream out;out<<L"UniversalDLSS5 diagnostics\r\n==========================\r\n";out<<L"Application: "<<selectedApplicationName()<<L"\r\nRoot PID: "<<g.rootPid<<L"\r\nBridge PID: "<<s.pid<<L"\r\nPrimary renderer PID: "<<(g.primaryRendererPid?g.primaryRendererPid:s.pid)<<L"\r\n";out<<L"State: "<<runtimeStateText(s.state)<<L"\r\nSource API: "<<graphicsApiText(s.api)<<L"\r\nRenderer route: "<<rendererRouteName(s.rendererRoute)<<L"\r\nCompatibility frontend: "<<(s.compatFrontendName[0]?s.compatFrontendName:L"(none)")<<L"\r\nGame architecture: "<<s.gameArchitectureBits<<L"-bit\r\nInterop: "<<(s.interopName[0]?s.interopName:compatInteropName(s.compatInterop).data())<<L"\r\nDetach mode: "<<detachModeName(s.detachMode)<<L"\r\nNeural API: "<<neuralApiText(s.neuralApi)<<L"\r\nExecution location: "<<neuralExecutionLocationLabel(s.neuralLocation)<<L"\r\nNR backend kind: "<<neuralBackendKindLabel(s.neuralBackendKind)<<L"\r\nBackend: "<<(s.backendName[0]?s.backendName:L"(not initialized)")<<L"\r\nRuntime folder: "<<g.runtimePath<<L"\r\n";out<<L"Swapchain: "<<swapFormatText(s.swapchainFormat)<<L" ("<<s.swapchainFormat<<L"), color-space="<<colorSpaceText(s.swapchainColorSpace)<<L" ("<<s.swapchainColorSpace<<L")\r\n";out<<L"HDR: "<<(s.hdrActive?L"ACTIVE":L"NO")<<L"; mode="<<hdrEncodingText(s.hdrEncoding)<<L"; supported="<<(s.hdrSupported?L"YES":L"NO")<<L"; transitions="<<s.hdrTransitionCount<<L"; paper-white="<<std::fixed<<std::setprecision(0)<<s.hdrPaperWhiteNits<<L" nits; peak="<<s.hdrMaxNits<<L" nits\r\n";if(s.lastPresentResult<0||s.deviceRemovedReason<0)out<<L"DXGI/D3D failure: Present=0x"<<std::hex<<std::uppercase<<(std::uint32_t)s.lastPresentResult<<L", deviceRemoved=0x"<<(std::uint32_t)s.deviceRemovedReason<<std::dec<<L"\r\n";if(s.nvidiaDriverVersion[0])out<<L"NVIDIA driver file version: "<<s.nvidiaDriverVersion<<(s.nvidiaDriverRisk?L" (known direct Feature-18 crash risk)":L"")<<L"\r\n";out<<L"Resolution: "<<s.width<<L" x "<<s.height<<L"\r\nColor path: "<<(s.colorPathWidth?s.colorPathWidth:s.width)<<L" x "<<(s.colorPathHeight?s.colorPathHeight:s.height)<<L"\r\nFrames: presented="<<s.presentedFrames<<L", pipeline="<<s.processedFrames<<L", neural="<<s.neuralFrames<<L", bypassed="<<s.bypassedFrames<<L"\r\nRoute counters: native="<<s.nativeFrames<<L", compatibility="<<s.compatFrames<<L", legacy copies="<<s.legacyCopies<<L", external-host="<<s.externalHostFrames<<L"\r\nQueue proof confidence: "<<s.queueProofConfidence<<L"%\r\nNeural active this frame: "<<(s.neuralActive?L"YES":L"NO")<<L"\r\nEstimated FPS: "<<std::fixed<<std::setprecision(1)<<s.estimatedFps<<L"\r\n";if(s.lastGpuMs>0.0f)out<<L"Feature-18 GPU time: "<<std::fixed<<std::setprecision(2)<<s.lastGpuMs<<L" ms\r\n";else out<<L"Feature-18 GPU time: unavailable\r\n";if(s.queueCapacity)out<<L"Neural queue: "<<s.queueDepth<<L" / "<<s.queueCapacity<<L" active slots (limit "<<s.queueLimit<<L")\r\n";out<<L"Neural passes: requested="<<s.neuralPassesRequested<<L", executed="<<s.neuralPassesExecuted<<L"\r\nFrame pacing: "<<pacingText(s.framePacingMode)<<L"; neural output age="<<s.neuralOutputAgeFrames<<L" frame(s) / "<<std::fixed<<std::setprecision(1)<<s.neuralOutputAgeMs<<L" ms; wait="<<s.pacingWaitMs<<L" ms\r\n";if(s.schedulerBackpressureFrames||s.reusedNeuralFrames)out<<L"Scheduler: pressure="<<s.schedulerBackpressureFrames<<L", reused="<<s.reusedNeuralFrames<<L", reused this frame="<<(s.reusedNeuralOutput?L"YES":L"NO")<<L"\r\n";if(s.flowName[0])out<<L"Motion path: "<<s.flowName<<L"\r\n";if(s.nativeMotionCandidateId||s.nativeMotionCandidateScore)out<<L"Native motion candidate: id="<<s.nativeMotionCandidateId<<L", score="<<s.nativeMotionCandidateScore<<L"\r\n";if(s.guideMotionWidth&&s.guideMotionHeight)out<<L"Game motion guide: "<<s.guideMotionWidth<<L" x "<<s.guideMotionHeight<<L", confidence="<<s.guideMotionConfidence<<L"%\r\n";if(s.guideDepthWidth&&s.guideDepthHeight)out<<L"Game depth guide: "<<s.guideDepthWidth<<L" x "<<s.guideDepthHeight<<L", confidence="<<s.guideDepthConfidence<<L"%\r\n";out<<L"Camera matrices: current="<<(s.cameraCurrentValid?L"YES":L"NO")<<L", previous="<<(s.cameraPreviousValid?L"YES":L"NO")<<L", confidence="<<s.cameraConfidence<<L"%\r\nTemporal history: "<<(s.temporalHistoryValid?L"VALID":L"RESET/INVALID");if(s.temporalReason[0])out<<L" ("<<s.temporalReason<<L")";out<<L"\r\nDebug view: "<<debugViewText(g.settings.debugView);if(g.settings.debugView!=DebugView::Final)out<<L" (diagnostic visualization, not final output)";out<<L"\r\n";if(s.depthName[0])out<<L"Depth guide: "<<s.depthName<<L"\r\n";if(s.guideAdapterName[0])out<<L"Game guide adapter: "<<s.guideAdapterName<<L"\r\n";if(s.guideFields[0])out<<L"Guide fields: "<<s.guideFields<<L"\r\n";out<<L"Depth source active: "<<(s.gameDepthActive?L"GAME":L"SYNTHETIC")<<L"; convention="<<(s.depthInverted?L"INVERTED / reversed-Z":L"NORMAL Z")<<L"\r\nNR control mask: "<<(s.controlMaskActive?L"ON":L"OFF")<<L"; temporal reset this frame: "<<(s.temporalResetThisFrame?L"YES":L"NO")<<L"\r\n";const auto ft=formatRuntimeFeatureDiagnostics(s.feature);if(!ft.empty())out<<L"Feature versions: "<<ft<<L"\r\n";if(s.lastResult)out<<L"Last result: 0x"<<std::uppercase<<std::hex<<std::setw(8)<<std::setfill(L'0')<<(std::uint32_t)s.lastResult<<std::dec<<L"\r\n";if(s.failureStage!=PipelineStage::None)out<<L"Failure stage: "<<pipelineStageLabel(s.failureStage)<<L"\r\n";out<<L"\r\nPipeline stages\r\n---------------\r\n"<<formatPipelineStages(s.stageMask,s.failureStage)<<L"\r\n\r\nBackend message\r\n---------------\r\n"<<(s.message[0]?s.message:L"(none)")<<L"\r\n";return out.str();}
...
fs::path logsDirectory(){wchar_t local[32768]{};DWORD n=GetEnvironmentVariableW(L"LOCALAPPDATA",local,_countof(local));return fs::path(n?std::wstring(local,n):g.appDir)/L"UniversalDLSS5"/L"logs";}

bool openLogsDirectory(){const auto dir=logsDirectory();std::error_code ec;fs::create_directories(dir,ec);if(ec)return false;const auto r=reinterpret_cast<INT_PTR>(ShellExecuteW(g.hwnd,L"open",dir.c_str(),nullptr,nullptr,SW_SHOWNORMAL));return r>32;}

std::wstring saveDiagnosticsToFile(){if(g.lastDiagnostic.empty())return{};fs::path dir=logsDirectory();std::error_code ec;fs::create_directories(dir,ec);SYSTEMTIME t{};GetLocalTime(&t);wchar_t name[96]{};swprintf_s(name,L"diagnostics-%04u%02u%02u-%02u%02u%02u.txt",t.wYear,t.wMonth,t.wDay,t.wHour,t.wMinute,t.wSecond);fs::path file=dir/name;std::ofstream f(file,std::ios::binary|std::ios::trunc);if(!f)return{};auto bytes=utf8(g.lastDiagnostic);f.write(bytes.data(),(std::streamsize)bytes.size());return f?file.wstring():std::wstring{};}
...
std::vector<ProcessInfo> attachTargetsFor(const ProcessInfo&root,bool followTree){if(!followTree)return{root};auto tree=processTree(root.pid);std::vector<ProcessInfo>out;out.reserve(tree.size());for(const auto&p:tree)if(shouldAttachRendererProcess(p.pid==root.pid,p.hasDxgi,p.rendererModules))out.push_back(p);return out;}
...
void maintenanceLoop(std::stop_token stop){
    ...
    while(!stop.stop_requested()){
        ...
        {std::scoped_lock lock(g.maintenanceMutex);root=g.maintenanceRoot;enabled=g.maintenanceEnabled;follow=g.maintenanceFollowTree;}
        if(enabled&&root.pid){
            const auto aliveProcesses=enumerateProcesses();
            const auto tracked=injectedSnapshot();
            bool anyAttachedAlive=false;
            for(const auto& p:aliveProcesses){
                if(p.pid==root.pid||tracked.contains(p.pid)){anyAttachedAlive=true;break;}
            }
            if(!anyAttachedAlive){
                g.shared.requestUnload(true);
                g.autoDetachedPending.store(true,std::memory_order_release);
                {std::scoped_lock lock(g.maintenanceMutex);g.maintenanceEnabled=false;g.maintenanceRoot={};}
            }else{
...
void drawApplicationPage(float x,float&y,float w){beginPage(L"Application",L"Select a renderer process, validate the runtime, then attach the bridge.",x,y,w);
    float listH=std::min(330.f,std::max(190.f,54.f*(float)std::min<size_t>(g.entries.size(),6)+70.f));auto c=card(x,y,w,listH);float cx=x+18,cy=y+16,cw=w-36;drawText(L"Target application",D2D1::RectF(cx,cy,cx+260,cy+24),g.headingFormat.Get(),g.textBrush.Get());button(g.refreshingApps?L"Refreshing...":L"Refresh",x+w-220,cy-2,108,32,BTN_REFRESH,false,!g.refreshingApps);toggle(L"Show all",g.showAllProcesses,x+w-100,cy-4,82,TGL_SHOW_ALL);cy+=42;
    const float listTop=cy;const float listBottom=c.bottom-14.f;g.appListViewport=D2D1::RectF(cx,listTop,cx+cw,listBottom);const float viewportH=std::max(1.f,listBottom-listTop);const int visibleRows=appPickerVisibleRows(viewportH,48.f);g.appListScroll=ensureAppPickerSelectionVisible(g.appListScroll,g.selectedIndex,g.entries.size(),visibleRows);
    ...
    else{g.target->PushAxisAlignedClip(g.appListViewport,D2D1_ANTIALIAS_MODE_PER_PRIMITIVE);const int first=clampAppPickerScroll(g.appListScroll,g.entries.size(),visibleRows);const int last=std::min<int>((int)g.entries.size(),first+visibleRows);float rowY=listTop;for(int i=first;i<last;++i){auto&e=g.entries[(size_t)i];D2D1_RECT_F row=D2D1::RectF(cx,rowY,cx+cw-14.f,rowY+44.f);bool sel=i==g.selectedIndex;if(sel)fillRounded(row,8.f,g.accentSoftBrush.Get());drawText(e.displayName,D2D1::RectF(cx+12,rowY+5,cx+cw-204,rowY+24),g.bodyFormat.Get(),sel?g.accentBrush.Get():g.textBrush.Get());drawText(processDetail(&e),D2D1::RectF(cx+12,rowY+24,cx+cw-26,rowY+42),g.smallFormat.Get(),g.mutedBrush.Get());addHit(row,HitKind::AppRow,0,i);rowY+=48.f;}g.target->PopAxisAlignedClip();if((int)g.entries.size()>visibleRows){const float trackTop=listTop+2.f,trackBottom=listBottom-2.f,trackH=std::max(1.f,trackBottom-trackTop);const float thumbH=std::max(28.f,trackH*((float)visibleRows/(float)g.entries.size()));const int maxScroll=std::max(1,(int)g.entries.size()-visibleRows);const float t=(float)g.appListScroll/(float)maxScroll;const float thumbTop=trackTop+t*std::max(0.f,trackH-thumbH);D2D1_RECT_F track=D2D1::RectF(cx+cw-6.f,trackTop,cx+cw-2.f,trackBottom);D2D1_RECT_F thumb=D2D1::RectF(cx+cw-7.f,thumbTop,cx+cw-1.f,thumbTop+thumbH);fillRounded(track,2.f,g.borderBrush.Get());fillRounded(thumb,3.f,g.mutedBrush.Get());}}y+=listH+14;
...
void drawCompositionPage(float x,float&y,float w){beginPage(L"Composition",L"Final blend, HDR/SDR color handling and post-composite controls.",x,y,w);float cy;float h=930;card(x,y,w,h);cy=y+18;std::vector<std::wstring>hdr={L"Auto",L"Force SDR",L"Force HDR"};choiceRow(L"Color pipeline",hdr,(int)g.settings.hdrMode,x+18,cy,w-36,CH_HDR,L"Auto follows the actual DXGI swapchain color space. HDR10 PQ and scRGB are converted through an HDR-preserving neural proxy.");slider(SL_SHARPNESS,x+18,cy,w-36);slider(SL_EXPOSURE,x+18,cy,w-36);slider(SL_TEMPORAL,x+18,cy,w-36);slider(SL_TEXT,x+18,cy,w-36);slider(SL_UI,x+18,cy,w-36);slider(SL_MASK,x+18,cy,w-36);slider(SL_CLAMP,x+18,cy,w-36);slider(SL_REACTIVE,x+18,cy,w-36);slider(SL_EDGE,x+18,cy,w-36);slider(SL_PAPER,x+18,cy,w-36);slider(SL_TRANSFER,x+18,cy,w-36);slider(SL_COLOR,x+18,cy,w-36);y+=h+14;}
...
case WM_CREATE:g.hwnd=h;g.dpi=(float)GetDpiForWindow(h);loadControllerPreferences();rebuildPalette();applyTheme();refresh();if(g.selectedIndex>=0)selectChanged(g.selectedIndex);SetTimer(h,1,500,nullptr);g.maintenanceThread=std::jthread(maintenanceLoop);return 0;
...
case WM_MOUSEWHEEL:{POINT pt{GET_X_LPARAM(l),GET_Y_LPARAM(l)};ScreenToClient(h,&pt);float scale=96.f/g.dpi;float x=(float)pt.x*scale,y=(float)pt.y*scale;if(g.page==Page::Application&&contains(g.appListViewport,x,y)){const float vh=std::max(1.f,g.appListViewport.bottom-g.appListViewport.top);const int rows=appPickerVisibleRows(vh,48.f);g.appListScroll=scrollAppPicker(g.appListScroll,GET_WHEEL_DELTA_WPARAM(w),g.entries.size(),rows);}else{float step=(GET_WHEEL_DELTA_WPARAM(w)/120.f)*72.f;g.contentScroll-=step;clampScroll();}InvalidateRect(h,nullptr,FALSE);return 0;}
case WM_TIMER:if(w==2){const auto now=GetTickCount64();if(!appPickerRefreshFeedbackActive(g.refreshStartedMs,now)){g.refreshingApps=false;KillTimer(h,2);InvalidateRect(h,nullptr,FALSE);}return 0;}poll();return 0;
case WM_SETTINGCHANGE:if(g.settings.uiTheme==UiTheme::System)applyTheme();return 0;
case WM_DESTROY:KillTimer(h,1);KillTimer(h,2);{std::scoped_lock lock(g.maintenanceMutex);g.maintenanceEnabled=false;}if(g.maintenanceThread.joinable()){g.maintenanceThread.request_stop();g.maintenanceThread.join();}saveProfileIfDirty();saveControllerPreferences();g.shared.requestUnload(true);g.shared.setPrimaryRendererPid(0);g.target.Reset();releaseBrushes();g.d2dFactory.Reset();g.dwriteFactory.Reset();PostQuitMessage(0);return 0;

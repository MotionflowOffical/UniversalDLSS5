...
case WM_DESTROY:KillTimer(h,1);{std::scoped_lock lock(g.maintenanceMutex);g.maintenanceEnabled=false;}if(g.maintenanceThread.joinable()){g.maintenanceThread.request_stop();g.maintenanceThread.join();}saveProfileIfDirty();saveControllerPreferences();g.shared.requestUnload(true);g.shared.setPrimaryRendererPid(0);g.target.Reset();releaseBrushes();g.d2dFactory.Reset();g.dwriteFactory.Reset();PostQuitMessage(0);return 0;

...
HRESULT STDMETHODCALLTYPE hookPresent(IDXGISwapChain*s,UINT sync,UINT flags){HookCallScope call;if(!call.customWorkAllowed())return origPresent(s,sync,flags);process(s);setCrashStage(CrashStage::CallingPresent);const auto hr=origPresent(s,sync,flags);setCrashStage(CrashStage::PresentReturned);recordPresentResult(s,hr);setCrashStage(CrashStage::Idle);return hr;}
HRESULT STDMETHODCALLTYPE hookPresent1(IDXGISwapChain1*s,UINT sync,UINT flags,const DXGI_PRESENT_PARAMETERS*p){HookCallScope call;if(!call.customWorkAllowed())return origPresent1(s,sync,flags,p);process(s);setCrashStage(CrashStage::CallingPresent);const auto hr=origPresent1(s,sync,flags,p);setCrashStage(CrashStage::PresentReturned);recordPresentResult(s,hr);setCrashStage(CrashStage::Idle);return hr;}
HRESULT STDMETHODCALLTYPE hookResize(IDXGISwapChain*s,UINT count,UINT w,UINT h,DXGI_FORMAT f,UINT flags){HookCallScope call;if(!call.customWorkAllowed())return origResize(s,count,w,h,f,flags);{std::scoped_lock l(mx);auto it=swaps.find(s);if(it!=swaps.end()){if(it->second->p11)it->second->p11->reset();if(it->second->p12)it->second->p12->reset();it->second->backbuffersRegistered=false;it->second->colorDirty=true;}unregisterSwapchainBackbuffersLocked(s);if(primary==s)primary=nullptr;}refreshQueueTouchCapture();return origResize(s,count,w,h,f,flags);}
...
void removeHooks(){
 RuntimeStatus st{};st.pid=GetCurrentProcessId();st.state=RuntimeState::Unloading;st.lastTickMs=GetTickCount64();wcscpy_s(st.message,L"unloading bridge: disabling hooks");if(ctl)ctl->writeStatus(st);
 beginHookUnload();
 const auto disabled=MH_DisableHook(MH_ALL_HOOKS);
 if(disabled!=MH_OK&&disabled!=MH_ERROR_NOT_CREATED&&disabled!=MH_ERROR_DISABLED){logAttachStage(AttachLogStage::Failure,L"MinHook disable reported an error during detach; waiting for active callbacks before teardown");}
 while(!waitForHookIdle(std::chrono::milliseconds(5000))){
  wchar_t detail[160]{};swprintf_s(detail,L"Detach waiting for %u in-flight hook callback(s) to finish",activeHookCalls());logAttachStage(AttachLogStage::Unloading,detail);
 }

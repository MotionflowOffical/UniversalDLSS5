...
struct SwapCtx{ComPtr<ID3D12CommandQueue> q12;ComPtr<ID3D12CommandQueue> queueCandidate;PresentQueueProofState queueProof{};std::unique_ptr<gpu::D3D11Pipeline> p11;std::unique_ptr<gpu::D3D12On12Pipeline> p12;GenerationTracker resetGeneration;GenerationTracker neuralRetryGeneration;SafeAttachState safeAttach{};GraphicsApi api{GraphicsApi::Unknown};bool d11TrackersInstalled{},d12TrackersInstalled{},guideHooksLogged{},safeLogged{},neuralLogged{},queueProofLogged{},backbuffersRegistered{};std::uint64_t lastGuideHookProbeFrame{};std::uintptr_t pipelineQueueId{};std::uint32_t width{},height{};std::uint64_t frames{},processed{},bypassed{},neural{},lastTick{};double fps{};};
...
HRESULT STDMETHODCALLTYPE hookPresent(IDXGISwapChain*s,UINT sync,UINT flags){process(s);setCrashStage(CrashStage::CallingPresent);const auto hr=origPresent(s,sync,flags);setCrashStage(CrashStage::PresentReturned);setCrashStage(CrashStage::Idle);return hr;} 
HRESULT STDMETHODCALLTYPE hookPresent1(IDXGISwapChain1*s,UINT sync,UINT flags,const DXGI_PRESENT_PARAMETERS*p){process(s);setCrashStage(CrashStage::CallingPresent);const auto hr=origPresent1(s,sync,flags,p);setCrashStage(CrashStage::PresentReturned);setCrashStage(CrashStage::Idle);return hr;} 
HRESULT STDMETHODCALLTYPE hookResize(IDXGISwapChain*s,UINT count,UINT w,UINT h,DXGI_FORMAT f,UINT flags){{std::scoped_lock l(mx);auto it=swaps.find(s);if(it!=swaps.end()){if(it->second->p11)it->second->p11->reset();if(it->second->p12)it->second->p12->reset();it->second->backbuffersRegistered=false;}unregisterSwapchainBackbuffersLocked(s);if(primary==s)primary=nullptr;}return origResize(s,count,w,h,f,flags);} 

...
[numthreads(8,8,1)]void CSMain(uint3 id:SV_DispatchThreadID){
 if(id.x>=Width||id.y>=Height)return;int2 p=id.xy;float4 n=Neural.Load(int3(p,0)),c=Current.Load(int3(p,0)),m=Mask.Load(int3(p,0));
 int2 l=int2(max(p.x-1,0),p.y),r=int2(min(p.x+1,(int)Width-1),p.y),u=int2(p.x,max(p.y-1,0)),d=int2(p.x,min(p.y+1,(int)Height-1));
 float3 blur=(Neural.Load(int3(l,0)).rgb+Neural.Load(int3(r,0)).rgb+Neural.Load(int3(u,0)).rgb+Neural.Load(int3(d,0)).rgb)*.25;
 float3 sharp=n.rgb+(n.rgb-blur)*Sharpness;float mixN=saturate(TemporalStrength*(1-m.y*HistoryClamp));float3 processed=lerp(c.rgb,sharp,mixN);processed=lerp(processed,c.rgb,saturate(m.a));
 float3 outc=processed;
 if(DebugView==1)outc=c.rgb;
 else if(DebugView==2)outc=(id.x<(uint)(DebugSplit*Width))?c.rgb:processed;
 else if(DebugView==3)outc=saturate(abs(processed-c.rgb)*4.0);
